...
# Вариант 2. Более алгоритмический, чем раньше, по комментариям

b = np.random.randint(16, size=(3,5)).astype('uint8')

#Переставим минимальное значение в колонках на первое место
for i in b: # идем по колонкам массива, для этого его перевернули - получаем список
    for k in range(len(i)): #проходимся по всей длине списка
        mini = k # задаем первое минимальное значение индексу минимального значения

        for j in range(k + 1, len(i)): #проходимся по всей длине списка, но начинаем с k + 1
            if i[j] < i[mini]: # если находим значение меньше минимального
                mini = j # присваиваем новое минимальное значение
                l_min_l.append(i[mini])
        i[mini], i[k] = i[k], i[mini]
