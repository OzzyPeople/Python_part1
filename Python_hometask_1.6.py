...
if __name__ == "__main__":
    print(f'размер в байтах алгоритма #1 - {asizeof.asizeof(HexOperation(num_first, num_second) + HexOperation(num_first, num_second))}')
    print (f'размер в байтах алгоритма #2 - {asizeof.asizeof(sum16(s))}')

'''
Резюме  - разница между первым и вторым вариантом составляет 2.6 MiB. По данным profile это происходит
только за счет самого профайлера. Сам алгоритм оценки памяти тяжелее для варианта 2, чем для 1-го.
А так разница в коде неочевидна, кроме его длины :)Размер алгоритмов в байтах - 256 .
Количество ссылок на оба алгоритма -  1.

версию Python 3.7 и разрядность ОС - 64.
'''
